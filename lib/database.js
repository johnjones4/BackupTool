...
class Database {
  ...
  install(callback) {
    this.db.exec('CREATE TABLE IF NOT EXISTS File (path TEXT PRIMARY KEY, modified INTEGER NOT NULL, backedup INTEGER NOT NULL, priority INTEGER NOT NULL)',callback);
  }
  ...
  touchFile(absPath,fsModifiedTime,callback) {
    ...
    this.db.all('SELECT modified FROM File WHERE path = ?',[absPath],function(err,rows) {
      if (err) {
        ...
        _this.db.run('INSERT INTO File (path,modified,backedup,priority) VALUES (?,?,0,0)',[absPath,fsModifiedTime],function(err) {
          callback(err,true);
        });
      } else if (rows && rows[0].modified < fsModifiedTime) {
        _this.db.run('UPDATE File SET modified = ?, priority = 0 WHERE path = ?',[fsModifiedTime,absPath],function(err) {
          callback(err,true);
        });
      } else {
        callback(null,false);
  ...
  getNextFileForBackup(done) {
    ...
    async.waterfall([
      function(callback) {
        _this.db.all('SELECT path FROM File WHERE modified >= backedup ORDER BY priority LIMIT 1',[],function(err,rows) {
          if (err) {
            callback(err);
          } else if (rows && rows.length > 0) {
