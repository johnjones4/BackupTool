...
class Database {
  ...
  getBackupStatus(done) {
    ...
    async.waterfall([
      function(callback) {
        _this.db.all('SELECT count(*) as c FROM File WHERE modified >= backedup',[],function(err,rows) {
          if (err) {
            callback(err);
          } else {
      ...
      function(part,callback) {
        _this.db.all('SELECT count(*) as c FROM File',[],function(err,rows) {
          if (err) {
            callback(err);
          } else {
            callback(null,part,rows[0].c);
          }
        });
      }
    ],done);
  ...
  getAllFiles(done) {
    this.db.all('SELECT path,modified,backedup,priority,size FROM File ORDER BY priority,size',[],done);
  }

  removeFileFromDatabase(path,done) {
    this.db.run('DELETE FROM File WHERE path=?',[path],done);
  }
